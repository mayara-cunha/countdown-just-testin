...
const newTimer = (eventName, timer2) => {
    const newTimerList = document.createElement("li");
    newTimerList.innerHTML = `
        <h4 id="event">${eventName}</h4>
        <div id="timer${count}" class="cronometro">         
            
        </div>     
    `

    const newAlarmSetter = document.createElement("div");
    newAlarmSetter.innerHTML = `
    <div>
        <hr>
        <audio id="audio" src="alarm_clock.mp3"></audio>
    </div>
    <div class="custom-control custom-switch">
        <input type="checkbox" name="checkbox" class="custom-control-input" id="customSwitches">
        <label class="custom-control-label" for="customSwitches">set the alarm</label>
    </div>
    `

    document.getElementById("timersList").appendChild(newTimerList);
    document.getElementById("timersList").appendChild(newAlarmSetter)
    let idTimer = `timer${count}`
    console.log("idtimer", idTimer)
    display2 = document.getElementById(idTimer);
    display = display2;
    timer(display2);
    count++
} 
...
const timer = (display2) => {
    ...
    let interval = setInterval(() => {
        ...
        addZeroBefore(days, hours, minutes, seconds, display2);
...
const addZeroBefore = (days, hours, minutes, seconds, display2) => {
    hours = hours < 10 ? "0" + hours : hours;
    minutes = minutes < 10 ? "0" + minutes : minutes;
    seconds = seconds < 10 ? "0" + seconds : seconds;

    //alterando o valor html do display pra mudar o cronometro na tela
    display2.innerHTML = `${days}d : ${hours}h : ${minutes}m : ${seconds}s`;
}
